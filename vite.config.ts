...
/**
 * Generates `/pwa-config.js` (consumed by `public/sw.js`) with the PWA cache
 * version read from `VITE_APP_VERSION`. Keeping the version in one generated
 * file lets the service worker cache-bust without hardcoding a build string in
 * the worker source.
 */
const pwaSwConfigPlugin = (): Plugin => {
  ...
  const buildScript = () =>
    [
      '// Auto-generated by mathpulse-pwa-sw-config (vite.config.ts).',
      '// Read by /sw.js to version the app-shell cache.',
      `self.__MATH_PULSE_PWA__ = Object.freeze({ version: ${JSON.stringify(swVersion)}, assets: ${JSON.stringify(buildAssets)} });`,
      '',
    ].join('\n');

  return {
    name: 'mathpulse-pwa-sw-config',
    config(_userConfig, configEnv) {
    ...
    generateBundle(_options, bundle) {
      buildAssets = Object.keys(bundle)
        .filter((fileName) => fileName.startsWith('assets/') && /\.(?:js|css|woff2?|png|jpg|webp|svg)$/.test(fileName))
