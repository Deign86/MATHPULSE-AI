        ...
        {items.map(item => {
          ...
          return (
            <div key={item.id} className="flex flex-col gap-1">
              ...
              <p className="text-xs font-bold text-slate-700 text-center line-clamp-2 my-0.5">{item.name}</p>
              {isLocked && !isExclusive && !item.isReward && (<motion.button initial={{ opacity: 0 }} animate={{ opacity: 1 }} onClick={(e) => handlePurchaseItem(e, item.id, item.price || 0)} disabled={purchasingItemId === item.id} className="w-full py-2 px-2.5 min-h-[38px] bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white text-xs font-bold flex items-center justify-center gap-1.5 rounded-lg shadow-sm transition-all disabled:opacity-70">{purchasingItemId === item.id ? (<motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1 }}><ShoppingBag size={14} /></motion.div>) : (<><ShoppingBag size={14} /> {item.price} XP</>)}</motion.button>)}
              {canPreview && !item.setLayers && (<button onClick={() => handlePreview(cat, item.id)} disabled={isPreviewActive} className={`w-full py-2 px-2.5 min-h-[38px] text-xs font-bold flex items-center justify-center gap-1.5 rounded-lg border transition-colors ${isPreviewActive ? 'bg-slate-100 text-slate-400 border-slate-200 cursor-not-allowed' : 'bg-amber-50 hover:bg-amber-100 text-amber-600 border-amber-200 cursor-pointer'}`}><Star size={13} /> Preview</button>)}
              {canPreview && item.setLayers && (<button onClick={() => handlePreview(cat, item.id)} disabled={isPreviewActive} className={`w-full py-2 px-2.5 min-h-[38px] text-xs font-bold flex items-center justify-center gap-1.5 rounded-lg border transition-colors ${isPreviewActive ? 'bg-slate-100 text-slate-400 border-slate-200 cursor-not-allowed' : 'bg-amber-50 hover:bg-amber-100 text-amber-600 border-amber-200 cursor-pointer'}`}><Star size={13} /> Preview Set</button>)}
            </div>
          );
        })}
      </div>
