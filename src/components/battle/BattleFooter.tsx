    ...
    <div className="absolute bottom-0 left-0 right-0 w-full xl:max-w-[1400px] mx-auto px-2 sm:px-4 md:px-8 shrink-0 h-28 sm:h-32 md:h-48 flex justify-between items-end pb-0 pointer-events-none z-30">
      ...
      <div className="flex items-end gap-1.5 sm:gap-3 md:gap-6 relative pointer-events-auto">
         ...
         <div className="bg-black/40 backdrop-blur-xl border border-white/20 rounded-xl sm:rounded-2xl px-2 sm:px-4 py-2 sm:py-3 md:px-5 shadow-[0_8px_30px_rgba(0,0,0,0.5)] flex items-center gap-1.5 sm:gap-3 md:gap-4 mb-2 sm:mb-4 max-w-[130px] sm:max-w-[220px] md:max-w-[280px]">
            <div className="flex flex-col flex-1 min-w-0">
               <span className="text-white font-black text-xs sm:text-base md:text-lg truncate tracking-wide">{studentProfile?.name || 'Player'}</span>
               <span className="text-[9px] sm:text-xs md:text-sm text-white/50 font-bold uppercase tracking-wider">Level {studentProfile?.level || 1}</span>
            </div>
            <div className="h-6 sm:h-10 w-px bg-white/20" />
            <div className="flex flex-col items-center justify-center shrink-0 w-6 sm:w-10 md:w-12">
               <motion.span 
                 animate={scorePulseTarget === 'player' ? { scale: [1, 1.4, 1], color: ['#fff', '#34d399', '#fff'] } : {}}
                 transition={{ duration: 0.5 }}
                 className="text-lg sm:text-2xl md:text-3xl font-black text-white leading-none"
               >
                 {activeMatch.scoreFor}
               </motion.span>
               <span className="text-[7px] sm:text-[9px] md:text-[10px] text-emerald-400 font-bold uppercase tracking-widest mt-0.5 sm:mt-1 leading-none">Score</span>
            </div>
         </div>
      </div>
      ...
      <div className="flex items-end gap-1.5 sm:gap-3 md:gap-6 relative flex-row-reverse pointer-events-auto">
         ...
         <div className="bg-black/40 backdrop-blur-xl border border-white/20 rounded-xl sm:rounded-2xl px-2 sm:px-4 py-2 sm:py-3 md:px-5 shadow-[0_8px_30px_rgba(0,0,0,0.5)] flex items-center gap-1.5 sm:gap-3 md:gap-4 mb-2 sm:mb-4 flex-row-reverse text-right max-w-[130px] sm:max-w-[220px] md:max-w-[280px]">
            <div className="flex flex-col flex-1 min-w-0">
               <span className="text-white font-black text-xs sm:text-base md:text-lg truncate tracking-wide">{activeMatch.opponentName || 'Anonymous'}</span>
               <span className="text-[9px] sm:text-xs md:text-sm text-rose-400 font-bold uppercase tracking-wider">{activeMatch.mode === 'bot' ? 'System Bot' : 'Challenger'}</span>
            </div>
            <div className="h-6 sm:h-10 w-px bg-white/20" />
            <div className="flex flex-col items-center justify-center shrink-0 w-6 sm:w-10 md:w-12">
               <motion.span 
               ...
               <span className="text-[7px] sm:text-[9px] md:text-[10px] text-rose-400 font-bold uppercase tracking-widest mt-0.5 sm:mt-1 leading-none">Score</span>
            </div>
         </div>
      </div>
    </div>
