# MathPulse AI: Frontend Generation Prompt

**Instructions for the AI Coding Agent:**
Act as an expert Frontend Developer specializing in React and Tailwind CSS. Your task is to build a pixel-perfect, responsive UI based on the attached image ("MATHPULSE UI (1).jpg"). Read the comprehensive architectural and stylistic requirements below before generating the code.
...
*   **Sticky Streak Bar:** The Navigation & Streak Bar sits directly below the header and must also remain fixed/sticky at the top.
*   **Sticky Footer:** The Bottom Footer must be fixed/sticky at the bottom.
*   **Scrollable Content:** The Middle Content Area (containing the question and answers) sits between the sticky top components and the sticky footer. It MUST have `overflow-y-auto` and `flex-1` so the user can scroll through the content while the header, streak bar, and footer remain frozen in place.
...
*   **Left Side (Resource Counters):**
    *   Two horizontal pill-shaped containers (`rounded-full`, `bg-white`, `shadow`).
...
### B. Navigation & Streak Bar (Sticky Top)
*   **Placement:** Sits immediately below the purple header, overlapping the gray background.
*   **Layout:** Flex row, centered, `justify-between` or `gap-8`.
*   **Left/Right Buttons:** Circular white buttons (`bg-white`, `shadow`, `text-purple-600`) with left and right chevron/arrow icons.
*   **Center Streak Pill:**
    *   Large white pill (`bg-white`, `shadow`, `rounded-full`).
    *   Content inside: Orange Fire Icon + "5" | Red Target/Bullseye Icon + "1.25x" | Green inset pill containing "+ 50 pts".
...
1.  Use `lucide-react` for all icons (e.g., Heart, Key, Volume2, Maximize, Menu, ChevronLeft, ChevronRight, Flame, Target, HelpCircle).
2.  Structure the code into clear functional components (e.g., `<Header />`, `<StreakBar />`, `<QuestionArea />`, `<Footer />`).
