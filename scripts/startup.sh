...
if [ -n "${CURRICULUM_SOURCE_REPO_ID:-}" ] || find "${CURRICULUM_DIR:-/app/datasets/curriculum}" -type f \( -name '*.pdf' -o -name '*.md' \) ! -name 'README.md' -print -quit >/dev/null 2>&1; then
    python /app/scripts/ingest_curriculum.py || echo "Curriculum ingest skipped during startup"
else
    echo "No curriculum files present and CURRICULUM_SOURCE_REPO_ID is unset; skipping ingest"
fi
